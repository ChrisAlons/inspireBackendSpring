...
- V1 = schema, V2 = catalog seeds, V3 = procedimiento seeds, V4 = usuario table + admin seed, V5 = seed de prueba (paciente completo).
...
- **Spring Security + JWT**: all endpoints require `Authorization: Bearer <token>` except `/api/auth/**` and Swagger.
- **Login**: `POST /api/auth/login` with `{"email", "password"}` returns JWT + user info.
- **Admin seed**: `admin@inspire.com` / `admin123` (created in V4 migration).
- **UsuarioPrincipal**: record with `usuarioId`, `personaId`, `email`. Use `@AuthenticationPrincipal` in controllers to access the authenticated user.
- **JWT config**: `jwt.secret` and `jwt.expiration-ms` in `application.properties` (overridable via env vars `JWT_SECRET`, `JWT_EXPIRATION_MS`).
- `plan_tratamiento` state changes get `actorPersonaId` from `SecurityContext`, not from request body.
